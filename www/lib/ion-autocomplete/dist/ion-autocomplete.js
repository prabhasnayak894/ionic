    ...
    function ($ionicBackdrop, $ionicScrollDelegate, $document, $q, $parse, $interpolate, $ionicPlatform, $compile, $templateRequest) {
        return {
            ...
            bindToController: {
                ...
                selectItemsLabel: '@',
                selectedItemsLabel: '@',
                showSelectedItemsLabel: '@',
                showSelectItemsLabel: '@'
            },
            ...
            controller: ['$attrs', '$timeout', '$scope', function ($attrs, $timeout, $scope) {
                ...
                $timeout(function () {
                    ...
                    controller.showSelectItemsLabel = valueOrDefault(controller.showSelectItemsLabel, true);
                    ...
                    controller.showSelectedItemsLabel = valueOrDefault(controller.showSelectedItemsLabel, true)
                });
            ...
            link: function (scope, element, attrs, controllers) {
                ...
                var template = [
                    ...
                    '<div class="bar bar-header item-input-inset bar-calm dark custom-nav">',
                    ...
                    '<button class="ion-autocomplete-cancel button button-light" ng-click="viewModel.cancelClick()">{{viewModel.cancelLabel}}</button>',
                    '</div>',
                    '<ion-content class="has-header container-bg padding">',
                    '<ion-item ng-if="viewModel.showSelectedItemsLabel == true" class="item-divider">{{viewModel.selectedItemsLabel}}</ion-item>',
                    ...
                    '</ion-item>',
                    '<ion-item ng-if="viewModel.showSelectItemsLabel == true" class="item-divider" ng-if="viewModel.searchItems.length > 0">{{viewModel.selectItemsLabel}}</ion-item>',
                    '<p class="no-records" ng-if="viewModel.searchItems.length == 0 && viewModel.searchQuery">No records found</p>',
                    '<div ng-repeat="item in viewModel.searchItems" >',
                        '<button class="button button-block {{($index % 2 == 0) ? \'even\' : \'odd\'}}" ng-click="viewModel.selectItem(item)">',
                            '{{viewModel.getItemValue(item, viewModel.itemViewValueKey)}}',
                            '<p ng-bind-html="item.description"></p>',
                            '<a href="javascript: void(0);" class="button button-small button-light button-outline">Select Point</a>',
                        '</button>',
                    '</div>',
                    '</ion-content>',
                    '</div>'
